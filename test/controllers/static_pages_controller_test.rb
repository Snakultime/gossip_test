...
class StaticPagesControllerTest < ActionDispatch::IntegrationTest
  ...
  setup do
    @user = User.create(last_name: 'foo1', first_name: 'foo1', email: 'foo@foo.foo', password: 'foo1', password_confirmation: 'fool1')
  end

  test 'the navbar display the logout and club links if user is logged in' do
    ...
    print "\nTest successful: 'the navbar display the logout and club links if user is logged in'"
  end
  ...
  test 'the club page is accessible to logged in user' do
    assert is_logged_in? @user
    get users_path
    assert_response :success
    print "\nTest successful: 'the club page is accessible to logged in user'"
  end
  ...
  test 'the private club page is NOT accessible if no one is logged' do
    ...
    print "\nTest successful: 'the club page is NOT accessible if no one is logged'"
  end

  test 'club page displays the list of all the users' do
    is_logged_in? @user
    ...
    print "\nTest successful: 'club page displays the list of all the users'"
  end
